...
class TransactionFormCubit extends Cubit<TransactionFormState> {
  ...
  Future<void> init({
    ...
    final List<CategoryModel> categories = (await categoryService.getGrouped(
      ...
      (c) => c.identifier != 'balance_transfer' || c.id == transaction?.categoryId,
    ).toList();
      ...
      emit(
        (state as TransactionFormInitial).copyWith(
          ...
          categories: loadCategories
              ? (await categoryService.getGrouped(
                  type: type,
                )).where((c) => c.identifier != 'balance_transfer').toList()
              : null,
          type: type,
          walletId: walletId,
  ...
  Future<bool> _validationForm(
    ...
    if (form.type == TransactionType.transfer && form.toWalletId == null) {
      errors['to_wallet_id'] = errorMessages['to_wallet_is_required']!;
    } else if (form.type == TransactionType.transfer &&
        form.toWalletId == form.walletId) {
      errors['to_wallet_id'] = errorMessages['to_wallet_must_be_different']!;
    }
